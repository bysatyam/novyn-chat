...
io.on("connection", (socket) => {
  ...
  socket.on("change_username", (payload, callback) => {
    const respond = typeof callback === "function" ? callback : (data) => socket.emit("change_username_result", data);
    const userKey = socket.data.userKey;
    if (!userKey) {
    ...
    const requested = normalizeHandleInput(payload?.newUsername || payload?.username || payload);
    if (!requested) {
      respond({ ok: false, message: "Invalid username. Use letters, numbers, or underscores." });
      return;
    }
    if (requested.length < 3 || requested.length > 24) {
      respond({ ok: false, message: "Username must be between 3 and 24 characters." });
      return;
    }
    const newKey = normalizeName(requested);
    if (newKey === userKey) {
      respond({ ok: false, message: "This is already your current username." });
      return;
    }
    if (users.has(newKey) && users.get(newKey)?.isRegistered) {
      respond({ ok: false, message: "This username is already taken. Try another." });
      return;
    }

    // Remap user in users map
    users.delete(userKey);
    user.username = requested;
    users.set(newKey, user);

    // Update socket session
    socket.data.userKey = newKey;
    ...
    emitFriendList(newKey);
    emitDiscoverOnlineToAll();
    schedulePersist();

    respond({ ok: true, newUsername: requested });
    socket.emit("profile_updated", {
      username: requested,
      displayName: user.displayName,
      bio: user.bio,
  ...
  socket.on("add_friend", (rawFriendName) => {
    ...
    let friend = users.get(friendKey) || findUserByEmail(friendName);
    if (!friend) {
      friend = getOrCreateUser(friendName);
    }
    const resolvedFriendKey = normalizeName(friend.username || friendKey);

    if (isBlockedBy(me, resolvedFriendKey)) {
    ...
    if (me.friends.has(resolvedFriendKey)) {
      socket.emit("error_message", { message: "Already friends." });
      return;
    }

    if (me.requests.has(resolvedFriendKey)) {
      me.requests.delete(resolvedFriendKey);
      me.friends.add(resolvedFriendKey);
      friend.friends.add(userKey);
      initializeUnreadPair(userKey, resolvedFriendKey);

      emitFriendList(userKey);
      emitFriendList(resolvedFriendKey);
      emitRequests(userKey);

      const friendSocket = onlineUsers.get(resolvedFriendKey);
      if (friendSocket) {
    ...
    if (friendSocket) {
      ...
      emitRequests(resolvedFriendKey);
    }

    schedulePersist();
  });

  socket.on("cancel_friend_request", (rawFriendName) => {
    const userKey = socket.data.userKey;
    if (!userKey) return;

    const friendName = toDisplayName(rawFriendName);
    const friendKey = normalizeName(friendName);
    const friend = users.get(friendKey) || findUserByEmail(friendName);
    if (!friend) return;

    friend.requests.delete(userKey);
    const resolvedFriendKey = normalizeName(friend.username || friendKey);
    emitRequests(resolvedFriendKey);
    socket.emit("friend_request_cancelled", { to: friend.username });
    schedulePersist();
  });

  socket.on("reject_friend", (rawFriendName) => {
    const userKey = socket.data.userKey;
    if (!userKey) return;
    ...
    emitRequests(userKey);
    socket.emit("friend_request_rejected", { from: friendName });
    schedulePersist();
  });
